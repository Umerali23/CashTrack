      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-ink-400 text-sm mt-1">
            {team.length} members collaborating
          </p>
        </div>
        <button
          onClick={openNew}
      ...
      {team.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {memberStats.map(({ member, earnings }) => {
            // Added a fallback '?' so it doesn't crash if name is missing
            ...
            return (
              <div
                key={member.id}
                className="glass rounded-2xl p-5 group hover:border-ink-500 transition-all duration-300"
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`h-14 w-14 rounded-2xl bg-gradient-to-br ${member.avatarColor} flex items-center justify-center text-white font-bold text-lg shadow-lg`}
                  >
                    {initials}
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-ink-700/50 cursor-pointer"
                    >
                ...
                {member.email && member.password && (
                  <div className="mb-4 p-3 rounded-xl bg-ink-900/60 border border-ink-600/50 space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-ink-400 font-mono truncate mr-2">
                        {member.password}
                      </span>
                      <button
                        onClick={() => copyToClipboard(member.password, "pass")}
                        className="text-ink-300 hover:text-white cursor-pointer"
                      >
                        {copiedField === "pass" ? (
                          <Check className="h-3 w-3 text-emerald-400" />
                ...
                <div className="pt-4 border-t border-ink-600/50">
                  ...
                  <div className="text-xl font-bold text-emerald-400">
                    {formatCurrency(earnings, currency, true)}
                  </div>
                </div>
              </div>
            );
      ...
      >
        <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 animate-scale-in">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between items-center bg-ink-900/50 p-2 rounded-lg">
                  ...
                  <button
                    onClick={() =>
                      copyToClipboard(generatedCreds.email, "modal-email")
                    }
                ...
                <div className="flex justify-between items-center bg-ink-900/50 p-2 rounded-lg">
                  <span className="text-ink-300 font-mono truncate mr-2">
                    {generatedCreds.password}
                  </span>
                  <button
                    onClick={() =>
                      copyToClipboard(generatedCreds.password, "modal-pass")
                    }
                    className="text-emerald-400 cursor-pointer"
                  >
                    {copiedField === "modal-pass" ? (
                      <Check className="h-4 w-4" />
                    ) : (
                      <Copy className="h-4 w-4" />
                    )}
              ...
              <p className="text-[10px] text-ink-400 mt-2">
                Share these with the team member. They can use them to log in
                immediately.
              </p>
            </div>
          )}
          ...
          <div>
            ...
            />
            {errors.role && (
              <div className="text-xs text-rose-400 mt-1">{errors.role}</div>
            )}
          </div>
          <div>
            <label className="text-xs font-semibold text-ink-300 mb-1.5 block">
