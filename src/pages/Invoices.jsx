...
export default function Invoices({ ctx, toast, user }) {
  ...
  const uninvoicedWork = useMemo(() => {
    ...
    transactions.forEach(tx => {
      ...
      if (tx.isAutoGenerated) return; // Don't auto-generate invoices from auto-generated transactions!
      
      if (!clientWork[tx.clientId]) clientWork[tx.clientId] = { items: [], total: 0, currency: tx.currency };
      clientWork[tx.clientId].items.push({ description: tx.description, amount: tx.amount, txId: tx.id, date: tx.date });
    ...
    return Object.entries(clientWork).map(([clientId, cData]) => ({
      clientId, client: clients.find(c => c.id === clientId), items: cData.items, total: cData.total, currency: cData.currency
    })).filter(cw => cw.client);
  ...
  const handleAutoGenerate = () => {
    ...
    setAutoGenerateOpen(false); setSelectedClient('');
  };
        ...
        <div className="glass rounded-2xl p-4 border border-emerald-500/20 bg-emerald-500/5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-ink-300 mb-3">{uninvoicedWork.reduce((sum, w) => sum + w.items.length, 0)} completed transactions across {uninvoicedWork.length} clients haven't been invoiced yet.</p>
