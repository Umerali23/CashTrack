  ...
  const uninvoicedWork = useMemo(() => {
    ...
    transactions.forEach(tx => {
      ...
      clientWork[tx.clientId].items.push({
        ...
        amount: tx.amount, txId: tx.id, date: tx.date
      });
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Invoice' : 'New Invoice'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-semibold text-ink-300 mb-1.5 block">Invoice Number</label><input type="text" value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} className="input" placeholder="INV-001" /></div>
            <div><label className="text-xs font-semibold text-ink-300 mb-1.5 block">Client</label><select value={form.clientId} onChange={(e) => setForm({ ...form, clientId: e.target.value })} className="input"><option value="">— Select Client —</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-2"><label className="text-xs font-semibold text-ink-300">Line Items</label><button onClick={addItemRow} className="text-xs text-emerald-400 hover:text-emerald-300 font-semibold cursor-pointer">+ Add Item</button></div>
            <div className="space-y-2">
              {form.items.map((item, index) => (<div key={index} className="flex gap-2"><input type="text" value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)} className="input flex-1" placeholder="Description" /><input type="number" value={item.amount} onChange={(e) => handleItemChange(index, 'amount', e.target.value)} className="input w-32" placeholder="Amount" />{form.items.length > 1 && (<button onClick={() => removeItemRow(index)} className="p-2 rounded-lg hover:bg-rose-500/10 text-rose-400 cursor-pointer"><Trash2 className="h-4 w-4" /></button>)}</div>))}
            </div>
        ...
        <div className="space-y-4">
          ...
          {selectedClient && (
            <div className="p-4 rounded-xl bg-ink-900/50 border border-ink-600/50">
              ...
              {uninvoicedWork.find(w => w.clientId === selectedClient)?.items.map((item, i) => (<div key={i} className="flex justify-between text-sm py-2 border-b border-ink-600/40 last:border-0"><span className="text-ink-300">{item.description}</span><span className="font-semibold tabular-nums">{formatCurrency(item.amount, uninvoicedWork.find(w => w.clientId === selectedClient).currency)}</span></div>))}
              <div className="flex justify-between mt-3 pt-3 border-t-2 border-ink-600"><span className="font-bold text-ink-100">Total</span><span className="font-bold text-emerald-400 tabular-nums">{formatCurrency(uninvoicedWork.find(w => w.clientId === selectedClient)?.total || 0, uninvoicedWork.find(w => w.clientId === selectedClient)?.currency)}</span></div>
            </div>
          )}
