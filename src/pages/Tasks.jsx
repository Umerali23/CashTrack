...
import { Plus, Search, Pencil, Trash2, CheckCircle2, Clock, AlertCircle, Calendar } from 'lucide-react';
...
const EMPTY_FORM = { title: '', description: '', clientId: '', assigneeId: '', status: 'pending', dueDate: new Date().toISOString().slice(0, 10) };
...
export default function Tasks({ ctx, toast, user }) {
  ...
  const openEdit = (t) => {
    setEditing(t);
    setForm({ title: t.title, description: t.description, clientId: t.clientId || '', assigneeId: t.assigneeId || '', status: t.status, dueDate: t.dueDate });
    setModalOpen(true);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">Tasks & Projects</h1>
          <p className="text-ink-400 text-sm mt-1">{user?.role === 'admin' ? 'Manage team workload' : 'Your assigned tasks'}</p>
        </div>
      ...
      <div className="glass rounded-2xl p-4 flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search tasks..." value={search} onChange={(e) => setSearch(e.target.value)} className="input pl-10" />
        </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-xs">
                        ...
                        {assignee && (
                          <span className="flex items-center gap-1.5 text-ink-300">
                            <span className={`h-4 w-4 rounded-full bg-gradient-to-br ${assignee.avatarColor} flex items-center justify-center text-[8px] text-white font-bold`}>{assignee.name[0]}</span>
                            {assignee.name}
                          </span>
                        )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Task' : 'New Task'}>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-ink-300 mb-1.5 block">Title</label>
            <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="input" placeholder="Design Homepage UI" />
          </div>
          <div>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button onClick={handleSave} className="btn-primary flex-1 bg-white text-ink-950 hover:bg-ink-100 hover:scale-[1.01]">{editing ? 'Save Changes' : 'Create Task'}</button>
          </div>
        </div>
      </Modal>
    </div>
