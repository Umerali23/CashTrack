...
// ✅ Accept user prop to handle role-based visibility
export default function Analytics({ ctx, user }) {
  const { 
    ...
    aggregates, 
    earningsByMember, 
    transactions // ✅ Use the filtered transactions list from the hook
  } = ctx;
  ...
  const { incomeTotal, expenseTotal, byCategory } = useMemo(() => {
    ...
    // ✅ Use filtered transactions so members only see their own categories
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-ink-400 text-sm mt-1">
          {user?.role === 'admin' 
            ? 'Deep dive into your freelance finances and team performance.' 
            : 'Deep dive into your personal financial performance.'}
        </p>
      </div>
      ...
      {/* ✅ Team Revenue Breakdown (Admin Only) */}
      {user?.role === 'admin' && (
        <div className="glass rounded-2xl p-6">
