      ...
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-64 flex-col glass border-r border-ink-600/60 z-30">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {NAV.map((item) => {
            ...
            return <button key={item.id} onClick={() => setPage(item.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer ${active ? 'bg-white text-ink-950 shadow-sm' : 'text-ink-300 hover:text-ink-100 hover:bg-ink-700/40'}`}><Icon className="h-[18px] w-[18px]" strokeWidth={active ? 2.3 : 2} />{item.label}</button>;
          })}
        </nav>
        <div className="px-4 pb-3">
          ...
          <div className="grid grid-cols-3 gap-1 p-1 rounded-xl bg-ink-900/60 border border-ink-600/60">
            {CURRENCIES.map((c) => (<button key={c.id} onClick={() => setDisplayCurrency(c.id)} className={`text-xs font-semibold py-1.5 rounded-lg transition-all cursor-pointer ${displayCurrency === c.id ? 'bg-ink-700 text-white shadow-sm' : 'text-ink-400 hover:text-ink-200'}`}>{c.label}</button>))}
          </div>
        </div>
        <div className="px-4 pb-4">
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium text-ink-300 hover:text-ink-100 hover:bg-ink-700/40 transition-all cursor-pointer">
        ...
        <div className="px-4 pb-5 border-t border-ink-600/50 pt-4">
          <div className="flex items-center gap-3 mb-3 px-1">
            <div className={`h-9 w-9 rounded-lg bg-gradient-to-br ${userColor} flex items-center justify-center text-white text-xs font-bold flex-shrink-0 shadow-md`}>{userInitials}</div>
            <div className="flex-1 min-w-0"><div className="text-sm font-semibold truncate">{user?.name || 'User'}</div><div className="text-[10px] text-ink-400 uppercase tracking-wider">{user?.role || 'member'}</div></div>
          </div>
          <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-rose-400 hover:bg-rose-500/10 transition-all cursor-pointer"><LogOut className="h-4 w-4" />Sign Out</button>
        </div>
      </aside>
      ...
      <div className="lg:hidden fixed top-0 inset-x-0 z-30 glass border-b border-ink-600/60">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <div className={`h-7 w-7 rounded-lg bg-gradient-to-br ${userColor} flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0`}>{userInitials}</div>
            ...
            <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="p-2 rounded-lg hover:bg-ink-700/50 cursor-pointer">{theme === 'dark' ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button>
          </div>
        </div>
      </div>
