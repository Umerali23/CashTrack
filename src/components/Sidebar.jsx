      ...
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-64 flex-col glass border-r border-ink-600/60 z-30">
          ...
          {NAV.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer ${active ? "bg-white text-ink-950 shadow-sm" : "text-ink-300 hover:text-ink-100 hover:bg-ink-700/40"}`}
              >
                <Icon
          ...
          <div className="grid grid-cols-3 gap-1 p-1 rounded-xl bg-ink-900/60 border border-ink-600/60">
            {CURRENCIES.map((c) => (
              <button
                ...
                className={`text-xs font-semibold py-1.5 rounded-lg transition-all ${displayCurrency === c.id ? "bg-ink-700 text-white shadow-sm" : "text-ink-400 hover:text-ink-200"}`}
              >
                {c.label}
              </button>
            ))}
        ...
        <div className="px-4 pb-5">
          ...
          >
            ...
            <div
              className={`h-5 w-9 rounded-full p-0.5 transition-colors ${theme === "dark" ? "bg-emerald-500" : "bg-ink-500"}`}
            >
              <div
                className={`h-4 w-4 rounded-full bg-white transition-transform ${theme === "dark" ? "translate-x-4" : ""}`}
              />
            </div>
          </button>
        </div>
          ...
          {NAV.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => setPage(item.id)}
                className={`flex flex-col items-center gap-1 py-3 text-[10px] font-semibold transition-colors ${active ? "text-emerald-400" : "text-ink-400"}`}
              >
