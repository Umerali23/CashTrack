...
export const useCashTrack = (user) => {
  ...
  // ✅ NEW: Automated Pipeline Functions
  const markInvoiceAsPaid = useCallback((invoiceId) => {
    setData((d) => {
      const inv = d.invoices.find(i => i.id === invoiceId);
      if (!inv || inv.status === 'paid') return d;

      // 1. Create the auto-generated income transaction
      const autoTx = {
        id: `t_auto_${Date.now()}`,
        type: 'income',
        amount: inv.total,
        ...
        assigneeId: inv.createdBy || null,
        category: 'Client Payment',
        date: new Date().toISOString().slice(0, 10),
        description: `Payment for Invoice ${inv.invoiceNumber}`,
        status: 'paid',
        isAutoGenerated: true, // Tag it so we know it's automated
        linkedInvoiceId: inv.id // Link it to the invoice
      };

      // 2. Update invoice status and add transaction
      const updatedInvoices = d.invoices.map(i => i.id === invoiceId ? { ...i, status: 'paid' } : i);
      return {
        ...d,
        invoices: updatedInvoices,
        transactions: [autoTx, ...d.transactions]
      };
    });
  }, []);

  const revertInvoiceStatus = useCallback((invoiceId, newStatus) => {
    setData((d) => {
      // Remove the auto-generated transaction linked to this invoice
      const filteredTx = d.transactions.filter(t => t.linkedInvoiceId !== invoiceId);
      // Revert invoice status
      const updatedInvoices = d.invoices.map(i => i.id === invoiceId ? { ...i, status: newStatus } : i);
      return { ...d, invoices: updatedInvoices, transactions: filteredTx };
    });
  }, []);
  ...
  const effectiveInvoices = useMemo(() => {
    ...
    const memberTxIds = new Set((data.transactions || []).filter(t => t.assigneeId === user.id).map(t => t.id));
  ...
  return { 
    ...
    markInvoiceAsPaid, revertInvoiceStatus, // ✅ Export new functions
